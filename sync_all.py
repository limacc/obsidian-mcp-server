...
SERVER_URL   = "https://obsidian-mcp-server-production-764d.up.railway.app"   # ← Railway 배포 후 URL로 교체
RELAY_SECRET = "bfc4b14e7a2812f80a8f1cae425393ca7c1d6f6dbf703bfa128aed6eae2a43a1"    # ← .env.example의 RELAY_SECRET과 동일
BATCH_SIZE   = 2    # 한 번에 전송할 파일 수 (Railway 무료: 50 이하 권장)
...
def collect_files(vault: Path) -> list[dict]:
    ...
    for f in vault.rglob("*"):
        ...
        if any(part.startswith(".") for part in f.parts):
            continue  # 숨김 폴더 제외
        if any(part in ("graphify-out", "node_modules", "__pycache__", ".venv", "cache") for part in f.parts):
            continue  # 캐시/빌드 폴더 제외
        try:
